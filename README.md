...
{
  "title": "칼럼 제목",
  "content": "<p>...</p> 형식의 HTML 또는 일반 텍스트 본문",
...
1. **HTTP Request 노드**
   ...
   - Body:
     ...
     - `content`: GPT가 생성한 전체 칼럼(HTML 문자열)
     - `summary`: (선택) 서머리
...
현재 예제는 `lib/posts.ts`에서 **Vercel Blob**을 사용해 각 글을 `posts/<slug>.json` 형태의 파일로 저장합니다.

- Vercel 대시보드의 **Storage → Blob → Create** 로 Blob 스토리지를 생성하고,
- 생성된 Blob 인스턴스를 이 프로젝트(`aicru-seven` 등)에 **Connect/Link** 하면, 별도 환경변수 설정 없이 바로 동작합니다.

실제 운영에서 규모가 커지면, Blob 외에 KV, Postgres, PlanetScale 등 다른 스토리지를 사용하는 것도 고려할 수 있습니다.
